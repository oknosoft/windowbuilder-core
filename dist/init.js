...
class CatSupplier_restrictionsManager extends CatManager {
...
get slave(){return this._getter('slave')}
set slave(v){this._setter('slave',v)}
get links(){return this._getter('links')}
set links(v){this._setter('links',v)}
get is_supplier(){return this._getter('is_supplier')}
set is_supplier(v){this._setter('is_supplier',v)}
get region(){return this._getter('region')}
set region(v){this._setter('region',v)}
...
class CatCnnsManager extends CatManager {
  ...
  nom_cnn(elm1, elm2, cnn_types, ign_side, is_outer, cnn_point) {
    ...
    const all = [];
    const thickness = art1glass && elm1.thickness;
    elm2.forEach(({nom, side}, index) => {
      for(const cnn of region_cache.get(nom)) {
        if((!cnn_types || cnn_types.includes(cnn.cnn_type)) && (cnn.sd1.is('any') || cnn.sd1 === side)) {
          ...
          if(is_nom || art1glass) {
            if(thickness && (cnn.tmin && cnn.tmin > thickness || cnn.tmax && cnn.tmax < thickness)) {
              continue;
            }
...
class CatInsert_bindManager extends CatManager {
  ...
  deposit({ox, scheme, spec, order}) {
    ...
    const {enm: {elm_types}, EditorInvisible: {ContourVirtual}, pricing, job_prm: {properties}, cat: {characteristics}} = $p;
    const new_rows = [];
...
get note(){return this._getter('note')}
set note(v){this._setter('note',v)}
get optimized(){return this._getter('optimized')}
set optimized(v){this._setter('optimized',v)}
  ...
  async clone(src) {
    ...
    if(!clone) {
      utils._mixin(dst._obj, others, null,
        'ref,date,number_doc,posted,_deleted,number_internal,production,planning,manager,obj_delivery_state,struct'.split(','));
